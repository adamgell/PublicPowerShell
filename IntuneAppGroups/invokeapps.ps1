    ...
    try {
        ...
        foreach ($AppDisplayName in $AppDisplayNames) {
            ...
            if ($matchResult.Match) {
                ...
                # Combine detected devices and expected device names
                $expectedDeviceNames = @("")
                    ...
                    if ($azureADDevice) {
                        ...
                        if ($azureADDevice.Id -notin $CurrentGroupMembers.Id) {
                            try {
                                ...
                                $devicesAdded++
                                $CurrentGroupMembers += $azureADDevice  # Add to current members to prevent duplicate attempts
                            }
                            catch {
...
# Example usage
$AppsToTrack = @("7-Zip 24.08 (x64 edition)", "Microsoft Edge", "Microsoft Teams", "Zoom", "Adobe Acrobat Reader DC")
