...
$approvalType = "automatic" # manual or automatic
...
# Connect to Graph
Connect-MgGraph -Scopes "DeviceManagementManagedDevices.ReadWrite.All", "Group.ReadWrite.All", "DeviceManagementConfiguration.ReadWrite.All", "GroupMember.ReadWrite.All"
...
$IntuneDevices = Get-MgBetaDeviceManagementManagedDevice -Filter "OperatingSystem eq 'Windows'" | Sort-Object -Property Model -Unique
...
foreach ($Device in $IntuneDevices) {
    ...
    if ($Device.Model -notin $ExcludedModels) {
        ...
        $ExistingGroup = Get-MgBetaGroup -Filter "DisplayName eq '$GroupName'"
        if (!$ExistingGroup) {
            ...
            $GroupResult = New-MgBetaGroup -BodyParameter $GroupParam
        }
        else {
...
$AllDriverGroups = Get-MgBetaGroup -Filter "startsWith(DisplayName,'$NamePrefix')"
