            ...
            if (Invoke-FileCertVerification -FilePath $SetupFilePath) {
                ...
                Try {
                    ...
                    if ($OfficeInstall.ExitCode -eq 0) {
                        Write-LogEntry -Value "M365 Apps installation completed successfully" -Severity 1
        
                        # Create the detection file
                        $DetectionFilePath = "C:\ProgramData\Microsoft\Apps\Remove.tag"
                        New-Item -Path $DetectionFilePath -ItemType File -Force | Out-Null
                        Write-LogEntry -Value "Created detection file at $DetectionFilePath" -Severity 1
                    }
                    else {
                        Write-LogEntry -Value "M365 Apps installation failed with exit code: $($OfficeInstall.ExitCode)" -Severity 3
                    }
                }
                catch [System.Exception] {
