...
$DetectionFilePath = "C:\ProgramData\Microsoft\M365Access\Installed.tag"
if (Test-Path $DetectionFilePath) {
    Write-LogEntry -Value "Detection file found at $DetectionFilePath" -Severity 1
    Write-Host "Detection file found. Microsoft 365 Apps installation confirmed." -ForegroundColor Green
    Write-LogEntry -Value "End Office Install detection logic, Uploading log" -Severity 1
    Exit 0
}
else {
    Write-LogEntry -Value "Detection file not found at $DetectionFilePath" -Severity 2
    Exit 1
}