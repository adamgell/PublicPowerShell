...
$OfficeInstall = "$($env:ProgramData)\Microsoft\OfficeInstall"
if (-not (Test-Path $OfficeInstall)) {
    Write-LogEntry "Creating OfficeInstall directory" -Severity 1
    Mkdir $OfficeInstall -Force
}
#endregion

#region Create tag file
# Creating tag file
Write-LogEntry "Creating tag file" -Severity 1
Set-Content -Path "$OfficeInstall\OfficeInstall.ps1.tag" -Value "Installed"
#endregion
