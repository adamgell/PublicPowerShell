...
Try {
    ...
    [String]$appVendor = ''
    [String]$appName = 'GlobalProtect Upgrader'
    [String]$appVersion = '6.2.4'
    [String]$appArch = ''
    [String]$appLang = 'EN'
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.0'
    [String]$appScriptDate = '09/16/2024'
    [String]$appScriptAuthor = 'Adam Gell (CDW)'
    ##*===============================================
    ...
    ## Variables: Script
    [String]$deployAppScriptFriendlyName = 'GlobalProtectUpgrader'
    [Version]$deployAppScriptVersion = [Version]'0.0.1'
    [String]$deployAppScriptDate = '09/16/2024'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        ## Show Welcome Message, close Internet Explorer if required, allow up to 10 deferrals, verify there is enough disk space to complete the install, and persist the prompt
        Show-InstallationWelcome -CloseApps 'ms-teams,Outlook,OneDrive,OneNote,PanGPA,PanGPS,winword,excel' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
        ...
        #Set-Service -Name PanGPS -StartupType Disabled
        #Get-Process | Where-Object { $_.Name -like 'pan*' } | ForEach-Object { Stop-Process -InputObject $_ -Force }
        ...
        # Define the regular expression for GUID
        $reGuid = '\{?(([0-9a-f]){8}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){4}-([0-9a-f]){12})\}?'

        # Get the data from the registry
        $data += Get-ChildItem $registryPath | Get-ItemProperty

        # Select the required properties from the data
        ...
        If (-not $useDefaultMsi) {
            Show-InstallationPrompt -Message 'Install successful. Please login to the VPN with your HCHB credentials.' -ButtonRightText 'OK' -Icon Information -NoWait
        }
